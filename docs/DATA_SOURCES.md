...
| **Launch Library 2** (The Space Devs) | Upcoming and just-flown space launches at their pads: time, status, provider, mission, orbit | Free API; "credit The Space Devs"; no explicit commercial terms | ❌ unconfirmed | "Launches: The Space Devs". Free tier is 15 calls/hour: the list is cached 1 h (20 min pause after errors) | 1 h |
...
| **Caltrans** CCTV (California DOT, CWWP2) | ~3,400 public highway cameras: location, direction, latest still image (via the API proxy), live HLS video where published (~25% of cameras list a stream; many are offline) | Public domain ("information presented on this website … is considered in the public domain") | ✅ | "Cameras: Caltrans". Free for integration over HTTPS; images are not retained upstream and Phoenix does not archive them. Camera list cached 1 h, snapshots 60 s. Live streams (`wzmedia.dot.ca.gov`, CORS `*`) play in the browser directly and are not relayed | Images every ~5 min; live video |
